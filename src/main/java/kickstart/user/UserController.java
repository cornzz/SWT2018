...
public class UserController {
		...
		ModelAndView registerNew(@ModelAttribute("form") @Validated(UserDto.RegistrationProcess.class) UserDto form, BindingResult result,
		...
		@PostMapping("/account")
		@PreAuthorize("isAuthenticated()")
		ModelAndView updateAccount(Model model, @ModelAttribute("form") @Validated(UserDto.UpdateAccProcess.class) UserDto form, BindingResult result,
		...
		@PreAuthorize("isAuthenticated()")
		ModelAndView changePassword(Model model, @ModelAttribute("form") @Validated(UserDto.ChangePassProcess.class) UserDto form, BindingResult result,
				@LoggedIn Optional<UserAccount> loggedIn) {
				try {
		...
		private void checkForm(UserDto form, BindingResult result, Optional<UserAccount> loggedIn, ProcessType processType) throws FormErrorException {
				...
				if (userManagement.mailExists(form.getEmail()) && (processType == ProcessType.REGISTRATION || !loggedIn.get().getEmail()
